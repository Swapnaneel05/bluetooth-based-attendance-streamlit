        ...
        if st.session_state.selected_class:
            ...
            for roll, student in st.session_state.students_data.items():
                if student["department"] == st.session_state.selected_class:
                    # Simulate finding a student device by name
                    # In a real web app, this would be a more complex check-in mechanism
                    if "present" in student["attendance"].lower(): # If student has clicked 'Give Attendance'
